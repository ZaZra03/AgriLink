...
<body class="relative">
    ...
    <form action="/AgriLink/modules/checkout.php" method="post">
        <div class="max-w-[965px] mx-auto pt-5 mb-96">
            ...
            <?php
$product_total = 0;
$service_fee = 11.06; // Fixed service fee
...
for($i = 0; $i < (count($records) / 2); $i++) {
    ...
    // Calculate subtotal (price after discount * quantity)
    $subtotal = round((int)$checkout_product['price'] - ((int)$checkout_product['price'] * ((int)$checkout_product['discount'] / 100))) * (int)$records['qty' . $i];
    ...
    <div class="bg-neutral w-full p-4 rounded-lg mb-5">
        ...
        <div class="flex items-center justify-between py-4">
            ...
            <h1>Price: <span class="font-semibold">₱' . number_format($subtotal) . '.00</span></h1>
        </div>
        ...
        <div class="flex flex-col items-end gap-2">
            <h1 class="mt-2 text-sm text-gray-700 flex items-center gap-2">
                Service Fee: <span class="text-gray-900 font-semibold">₱' . number_format($service_fee, 2) . '</span>
            </h1>
            <h1 class="my-2 font-semibold text-sm flex items-center gap-2">
                Order Total: <span class="text-primary text-2xl">₱' . number_format($subtotal + 38 + $service_fee, 2) . '</span>
            </h1>
        </div>
    </div>
...
?>
            ...
            <div class="grid grid-cols-3 w-full bg-neutral px-5 py-8 rounded mb-5">
                ...
                <div class="grid grid-cols-2 gap-2 py-5 border-b border-gray-300">
                    ...
                    <h1>Service Fee:</h1>
                    <h1 class=" text-right">₱11.06</h1>
                    <h1>Total Payment:</h1>
                    <h1 class="text-3xl text-right text-primary font-semibold">₱<?php echo number_format(11.06 + $product_total + (ceil(count($records) / 2) * 38), 2) ?></h1>
                </div>
